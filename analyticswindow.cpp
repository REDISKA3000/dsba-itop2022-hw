...
AnalyticsWindow::AnalyticsWindow(QWidget* parent,CustomProxy* derived) :
    QWidget(parent),
    uiAn(new Ui::AnalyticsWindow),
...
void AnalyticsWindow::constructPie(const int& index)
{
    ...
    {
        QModelIndex ind = _derivedMain->index(i,index);
    ...
    for (std::map<std::string,int>::iterator it = values.begin(); it != values.end(); ++it)
    {
        ...
        std::string key = it->first + "-" + QVariant(percentage).toString().toStdString() + "%";
        if (it->first.empty())
            continue;
        *series << new QPieSlice(key.c_str(), qreal(value));
